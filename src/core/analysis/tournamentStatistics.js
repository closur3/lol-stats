...
function collectMatchesByOverviewPage(rawMatches, tournament) {
  ...
  for (const match of rawMatches) {
    if (!match || typeof match !== "object" || Array.isArray(match)) {
      throw new Error(`${tournament.name} raw match must be an object`);
    }
    const pageMatches = matchesByPage.get(match.OverviewPage);
    if (!pageMatches) {
      throw new Error(`${tournament.name} match outside overviewPage scope: ${String(match.OverviewPage)}`);
    }
...
export function buildTournamentStatistics(rawMatches, tournament, resolveTeamName) {
  if (!Array.isArray(rawMatches)) throw new Error(`${tournament.name} rawMatches must be an array`);
  const overviewPages = getOverviewPageNames(tournament.overviewPages);
  if (!Array.isArray(tournament.participantGroups)) {
    throw new Error(`${tournament.name} participantGroups must be an array`);
  }

  const matchesByPage = collectMatchesByOverviewPage(rawMatches, tournament);
  const combinedAnalysis = parseTournamentMatches(rawMatches, resolveTeamName, tournament.name);
  const pageAnalyses = overviewPages.map(overviewPage => {
    ...
    return overviewPages.length === 1
      ? combinedAnalysis
      : parseTournamentMatches(pageMatches, resolveTeamName, `${tournament.name}:${overviewPage}`);
  });
