...
import { parseAllMatches } from "./matchParser.js";

const resolveName = (name) => name;
const tournamentArgs = [resolveName, "2026-07-10", "test", "TST", 0, {}];

describe("parseAllMatches", () => {
  it("keeps forfeits as finished results without counting them as full-length", () => {
    const { parsedMatches, stats } = parseAllMatches([{
  ...
  it("excludes a nullified result from every derived output", () => {
    const { parsedMatches, stats, scheduleMeta } = parseAllMatches([{
