...
function readActiveHomeIssue(home, artifactKey) {
  ...
  const expectedFields = ["tournament", "stats", "timeGrid"];
  if (homeFields.length !== expectedFields.length || expectedFields.some(field => !Object.hasOwn(home, field))) {
    return createSchemaIssue({ artifactKey, path: "$", kind: "invalid", expected: "fields tournament, stats, timeGrid", actual: homeFields.join(", ") });
  }
