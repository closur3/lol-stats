...
export async function rebuildArchiveSnapshot(env, tournament) {
  ...
  const matches = await fandomClient.fetchAllMatches(tournament.name, getOverviewPageNames(tournament.overviewPages));
  if (!matches || matches.length === 0) throw new Error("No matches found from Fandom API");

  await env["lol-stats-kv"].put(kvKeys.archive(tournament.name), JSON.stringify(buildArchiveSnapshot(tournament, matches)));
}
