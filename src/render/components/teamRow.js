...
export function buildTeamRow(teamStats, slug, sortMeta = {}) {
  ...
  const percentStyle = (value, strong = false) => `style="background:${color(value, strong)};color:${value !== null ? 'white' : emptyColor};font-weight:bold"`;
  ...
  return `<tr><td class="team-col team-clickable" onclick="openTeam('${slug}', '${safeName}')">${safeDisplayName}</td>` +
    ...
    `<td class="col-bo3-pct" data-bayes-tie="${bo3BayesTieBreakRate}" data-sample-size="${teamStats.bestOf3TotalMatchCount || 0}" ${percentStyle(bo3Rate, true)}>${pct(bo3Rate)}</td>` +
    ...
    `<td class="col-bo5-pct" data-bayes-tie="${bo5BayesTieBreakRate}" data-sample-size="${teamStats.bestOf5TotalMatchCount || 0}" ${percentStyle(bo5Rate, true)}>${pct(bo5Rate)}</td>` +
    ...
    `<td class="col-series-wr" ${percentStyle(winRate)}>${pct(winRate)}</td>` +
    `<td class="col-game" ${statStyle(teamStats.gameTotalCount)}>${gameText}</td>` +
    `<td class="col-game-wr" ${percentStyle(gameRate)}>${pct(gameRate)}</td>` +
