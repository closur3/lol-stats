...
import { localizeTimeGrid } from '../../utils/timeGridLocalizer.js';
...
export function buildTimeTable(regionGrid, displayTimeZone = "Asia/Shanghai") {
  const displayGrid = localizeTimeGrid(regionGrid, displayTimeZone);
  const hours = Object.keys(displayGrid).filter(key => key !== "Total" && !isNaN(key)).map(Number).sort((leftHour, rightHour) => leftHour - rightHour);
  ...
  [...hours, "Total"].forEach(hour => {
    ...
    for (let dayIndex = 0; dayIndex < TIME_GRID_COLUMN_COUNT; dayIndex++) {
      const cellData = displayGrid[hour][dayIndex] || { totalMatchCount: 0 };
